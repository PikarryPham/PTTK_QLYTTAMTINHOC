...
AS
BEGIN
	IF EXISTS (SELECT * FROM KYTHICCQT JOIN CCQT ON KYTHICCQT.KTQT_CCQT = CCQT.CCQT_ID WHERE CCQT.CCQT_TEN = @TENCCQT)
		BEGIN
			RETURN 1;
		END
	RETURN 0;
...
DECLARE @CODE INT, @MESS NVARCHAR(100)
EXEC PTTK_LayThongTinDKHocPhan '026060130',@RETURNCODE = @CODE OUTPUT, @RETURNMESS = @MESS OUTPUT ; 
SELECT @MESS AS N'Thông báo';
...
DECLARE @CODE INT, @MESS NVARCHAR(100)
EXEC PTTK_HuyDKHocPhan '026060130',2,'2021-05-16',@RETURNCODE = @CODE OUTPUT, @RETURNMESS = @MESS OUTPUT ; 
SELECT @MESS AS N'Thông báo';

/*Xem kết quả học phần*/
...
BEGIN TRANSACTION
	BEGIN TRY
		...
		-- Số lần thi lại
		--SELECT COUNT(*) FROM LICHTHILAI
		--JOIN DANGKYHOCPHAN
		--ON LICHTHILAI.LTL_DKHOCPHAN = DANGKYHOCPHAN.DKHP_ID
	END TRY
	BEGIN CATCH
		SET @RETURNCODE = 1;
		SET @RETURNMESS =  N'Có lỗi xảy ra'
