...
EXEC PTTK_KiemTraThongTinDauVao 'NVTN001','NVTT001',3,@RETURNCODE = @CODE OUTPUT, @RETURNMESS = @MESS OUTPUT; 
...
SELECT @THONGBAO AS N'Thông báo';
...
/*Lấy thông tin lịch thi lại*/
IF OBJECT_ID('PTTK_LayThongTinLichThiLai','P') IS NOT NULL
	DROP PROC PTTK_LayThongTinLichThiLai
GO
CREATE PROC PTTK_LayThongTinLichThiLai
@CMNDHOCVIEN VARCHAR(20),
@IDHOCPHAN INT = 1,
@THOIGIANDKHOCPHAN DATE = '2021-05-15',
@NGAYTHILAI NVARCHAR(100) OUTPUT ,
@MALICHTHILAI NVARCHAR(100) OUTPUT ,
@PHONGTHI NVARCHAR(100) OUTPUT ,
@THOIGIANBDTHI NVARCHAR(100) OUTPUT ,
@THGIANLAMBAI NVARCHAR(100) OUTPUT ,
@RETURNCODE INT OUTPUT, 
@RETURNMESS NVARCHAR(100) OUTPUT
AS
BEGIN TRANSACTION
	BEGIN TRY
		...
		BEGIN
			...
			RETURN;
		END
		-- Lấy ra id dkhocphan của thông tin đăng ký học phần
		DECLARE @IDDKHOCPHAN INT = (SELECT DKHP.DKHP_ID FROM DANGKYHOCPHAN AS DKHP WHERE DKHP.DKHP_IDHOCVIEN = @IDHOCVIEN AND DKHP.DKHP_NGAYDK = @THOIGIANDKHOCPHAN AND DKHP.DKHP_IDHP = @IDHOCPHAN)
		PRINT @IDDKHOCPHAN
		-- Thông tin lịch thi lại
		SET @MALICHTHILAI = (SELECT LICHTHILAI.LTL_ID FROM LICHTHILAI WHERE LICHTHILAI.LTL_DKHOCPHAN = @IDDKHOCPHAN);
		SET @NGAYTHILAI = (SELECT LICHTHILAI.LTL_NGAY FROM LICHTHILAI WHERE LICHTHILAI.LTL_DKHOCPHAN = @IDDKHOCPHAN);
		SET @PHONGTHI = (SELECT LICHTHILAI.LTL_PHONG FROM LICHTHILAI WHERE LICHTHILAI.LTL_DKHOCPHAN = @IDDKHOCPHAN);
		SET @THOIGIANBDTHI = (SELECT LICHTHILAI.LTL_TGBATDAU FROM LICHTHILAI WHERE LICHTHILAI.LTL_DKHOCPHAN = @IDDKHOCPHAN);
		SET @THGIANLAMBAI = (SELECT LICHTHILAI.LTL_TGLAMBAI FROM LICHTHILAI WHERE LICHTHILAI.LTL_DKHOCPHAN = @IDDKHOCPHAN);
		SET @RETURNCODE = 0;
		SET @RETURNMESS =  N'Xem lịch thi lại thành công'
		COMMIT TRANSACTION
	END TRY
	BEGIN CATCH
		SET @RETURNCODE = 1;
...
DECLARE @CODE INT, @MESS NVARCHAR(100), @MA NVARCHAR(100), @NGAYTHI NVARCHAR(100), @PHONG NVARCHAR(100), @BDTHI NVARCHAR(100), @LBAILAI NVARCHAR(100)
EXEC PTTK_LayThongTinLichThiLai '321743123',1,'2021-01-05',@NGAYTHILAI = @NGAYTHI OUTPUT, @MALICHTHILAI = @MA OUTPUT, @PHONGTHI = @PHONG OUTPUT, @THOIGIANBDTHI = @BDTHI OUTPUT, @THGIANLAMBAI = @LBAILAI OUTPUT,
@RETURNCODE = @CODE OUTPUT, @RETURNMESS = @MESS OUTPUT ; 
SELECT @NGAYTHI, @MA, @PHONG, @BDTHI, @LBAILAI;

SELECT LICHTHILAI.LTL_ID FROM LICHTHILAI WHERE LICHTHILAI.LTL_DKHOCPHAN = 1
